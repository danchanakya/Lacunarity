function  LacAnalysis2014()
	...
	Calc_Hist(resFolder, imagename, idata, dataHst, nHst);

	disp(['Otsu threshold']);
	Calc_OtsuThreshold(resFolder, imagename, idata);
...
%------------------------------------------------------------------------------
function lacNorm = NormalizedPlotnick(data, dataR, logOut)
	lacNorm = [];
	for i = 1:length(data)
...
%------------------------------------------------------------------------------
function Calc_Hist(resFolder, imagename, idata, dataHst, nHst)
	[sx sy]	= size(idata);
	minxy	= min(sx, sy);
	...
	%dataFilename = strcat(resFolder, imagename, '_1.csv');
...
%------------------------------------------------------------------------------
function Calc_OtsuThreshold(resFolder, imagename, idata)
	threshold = 109; %graythresh(idata);
	%trData0 = im2bw(idata,threshold);
	...
	%dataFilename = strcat(resFolder, imagename, '_otsu.csv');
	dataFilename = strcat(resFolder, imagename, '_otsu');
	SaveLacData(dataFilename, x, [lac1], [lacLeg]);
	...
	y3 = NormalizedPlotnick(lac1, lacR1, 1);  % log log
